#############################################################################
#	Algoritmo de prueba de predictibilidad de serie de tiempo de RNH 		#
#	implementa modelo de persistencia para serie de tiempo de headway		#
#############################################################################
...
# Carga el dataset
dataframe = read_csv('../../datos/datasetDefinitivoModelo2.tsv', engine = 'python', sep = "\t")
...
for i in range(len(dataset)):
	...
	if len(dataset_viajes) > 0:
		...
		if dataset_viajes[len(dataset_viajes)-1].id == id_lectura:
			dataset_viajes[len(dataset_viajes)-1].x = numpy.append(dataset_viajes[len(dataset_viajes)-1].x,[dataset[i][1:7]],axis=0)
			dataset_viajes[len(dataset_viajes)-1].y = numpy.append(dataset_viajes[len(dataset_viajes)-1].y,[dataset[i][7]],axis=0)
		#Agrega un nuevo viaje
		else:
			dataset_viajes.append(Viaje(id_lectura))
			dataset_viajes[len(dataset_viajes)-1].x = numpy.array([dataset[i][1:7]])
			dataset_viajes[len(dataset_viajes)-1].y = numpy.array([dataset[i][7]])
...
for i in range(len(train)):
	for j in range(len(train[i].x)):
		if i==0:
			deltas = [abs(train[i].x[j][2] - train[i].y[j])]
		else:
			deltas.append(abs(train[i].x[j][2] - train[i].y[j]))
...
for i in range(len(test)):
	for j in range(len(test[i].x)):
		if i==0:
			deltas = [abs(test[i].x[j][2] - test[i].y[j])]
		else:
			deltas.append(abs(test[i].x[j][2] - test[i].y[j]))
...
print("Modelo de persistencia (RNH)")
